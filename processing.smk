...
GENE_ANNO=config['gene_annotation']
...
rule all:
    input:
        ...
        velocyto=expand('{sample_folder}/cellcount/{sample}/velocyto/log.txt',sample_folder=SAMPLE_FOLDER,sample=SAMPLES),
...
rule velocyto:
    input:
        SAMPLE_FOLDER+'/cellcount/{sample}/outs/possorted_genome_bam.bam'
    output:
        SAMPLE_FOLDER+'/cellcount/{sample}/velocyto/log.txt'
    params:
        sample='{sample}',
        sample_folder=SAMPLE_FOLDER,
        nthread=NTHREAD,
        gene_anno=GENE_ANNO
    log:
        'log/velocyto_{sample}.log'
    shell:"""velocyto run10x {params.sample_folder}/cellcount/{params.sample} {params.gene_anno} -@ {params.nthread} > {log} 2>&1
    echo "velocyto run10x {params.sample_folder}/cellcount/{params.sample} {params.gene_anno} -@ {params.nthread} " > {params.sample_folder}/cellcount/{params.sample}/velocyto/log.txt
    """
