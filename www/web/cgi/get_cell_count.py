...
if cate=='TE':
    ...
    sql=f"select DATA_CELLUMAP.disease, DATA_CELLUMAP.cell_type, COUNT(*) AS number_of_cells FROM DATA_CELLUMAP INNER JOIN CELL_EXP_{table_id} ON DATA_CELLUMAP.CELL=CELL_EXP_7.CELL where CELL_EXP_{table_id}.{kw} >0 GROUP by disease, cell_type; "
    cursor.execute(sql)
